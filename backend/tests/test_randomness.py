...
# --- 2026-09 re-audit: an empty chain anchors nothing --------------------------------
#
# The genesis check sat AFTER the per-round loop as `if require_genesis and rounds:`, so an
# empty list skipped every check and the function returned True: a chain containing no
# rounds at all verified as a complete, genesis-anchored chain. verify_beacon_chain is the
# consumer-facing API named in SECURITY.md ("independently verifiable"), and /api/beacon
# serves the rounds a consumer feeds it — so anyone who can serve or relay that response
# could hand over `{"rounds": []}` and have the consumer conclude the beacon was intact.
#
# The sibling copy under oracles/oracles/platon already had the guard, with the reasoning
# written out. This is the fork catching up.

def test_an_empty_chain_is_not_a_genesis_anchored_chain():
    from platon.randomness import verify_beacon_chain

    assert verify_beacon_chain([]) is False
    assert verify_beacon_chain([], require_genesis=True) is False


def test_an_empty_chain_is_acceptable_only_as_a_rolling_window_check():
    """require_genesis=False is the documented "known window that omits genesis" mode."""
    from platon.randomness import verify_beacon_chain

    assert verify_beacon_chain([], require_genesis=False) is True
