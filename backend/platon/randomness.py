...
def verify_beacon_chain(
    ...
    # An empty list anchors NOTHING, and the genesis check used to sit after the loop as
    # `if require_genesis and rounds:` — so `verify_beacon_chain([])` skipped every check
    # and returned True: a chain with no rounds verified as a complete, genesis-anchored
    # one. This is the consumer-facing API SECURITY.md points at, and /api/beacon supplies
    # the rounds, so whoever serves or relays that response could hand over `{"rounds": []}`
    # and be believed. The sibling copy under oracles/oracles/platon already had this guard.
    if not rounds:
        return not require_genesis
    # Ahead of the loop on purpose: no round of an unanchored chain is worth verifying.
