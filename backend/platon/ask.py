...
#: Global, ALL-CALLERS ceiling on paid completions from this path.
#:
#: `answer()` is reachable from two doors — POST /api/ask (30/min per IP) and
#: platon.ask@v1 through /ai-market/v2/invoke (120/min per IP) — and neither requires
#: payment, so the cheaper-guarded door set the real rate. A per-IP limit is in any case the
#: wrong tool against a distributed caller, and that is not hypothetical: the operator's own
#: traffic analysis found a 72-address residential-proxy fleet. The sibling LLM path
#: (`oracle.generate_witness`) has had exactly this budget for the same reason; this one did
#: not. Over budget we fall through to `_fallback()` — a real answer built from live state,
#: just not a paid one.
_ASK_LLM_PER_MIN = max(1, int(os.environ.get("PLATON_ASK_LLM_PER_MIN", "20") or 20))
_ASK_WINDOW_S = 60.0
_ask_calls: deque[float] = deque()
_ask_lock = threading.Lock()


def _ask_llm_budget_available() -> bool:
    now = time.monotonic()
    with _ask_lock:
        while _ask_calls and _ask_calls[0] <= now - _ASK_WINDOW_S:
            _ask_calls.popleft()
        if len(_ask_calls) >= _ASK_LLM_PER_MIN:
            return False
...
async def answer(question: str, lang: str, engine) -> dict:
    ...
    safe_q = wrap_user_question(question)
    if settings.oracle_enabled and _ask_llm_budget_available():
        system = _system_prompt(lang, context)
        text = await _deepseek_chat(system, safe_q)
        if text:
            return {"answer": text, "source": "deepseek", "lang": lang}
        text = await _ollama_chat(system, safe_q)
        if text:
