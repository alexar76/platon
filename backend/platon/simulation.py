...
class SimulationEngine:
    ...
    #: Bounds on ``dream`` steps. Enforced HERE rather than at each entry point: /api/dream
    #: and the WebSocket handler both clamped to this range, but platon.dream@v1 on
    #: /ai-market/v2/invoke passed the caller's number straight through -- and the handler
    #: runs synchronously inside the request coroutine, so one anonymous request with
    #: steps=1e9 pinned the whole event loop (every route, every socket) in an RK2 loop.
    DREAM_MIN_STEPS = 10
    DREAM_MAX_STEPS = 120
    ...
    def dream(self, steps: int = 60) -> dict[str, Any]:
        try:
            ...
            steps = 60
        steps = min(max(steps, self.DREAM_MIN_STEPS), self.DREAM_MAX_STEPS)
