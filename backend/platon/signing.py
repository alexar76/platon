...
The manifest canonical (5-field: ``tools_hash`` AND ``by_hub_hash``) and the receipt
canonical (7-field) match ``aimarket_hub/aimarket_hub/signing.py`` exactly, so signatures
produced here verify against the live hub at modelmarket.dev.

The manifest canonical gained ``by_hub_hash`` when the hub started covering per-peer
trust_score and routing metadata with the signature. ``oracle_core`` was updated; both
copies of platon were not, so platon kept signing four fields while the hub verified five —
and the hub's crawler rejected every platon manifest with "Invalid manifest signature" and
indexed nothing. That is why modelmarket.dev showed a peer with capabilities_count 11 and
federated_capabilities 0: the peer record is written before the manifest is fetched, so the
count survived while the capabilities never arrived. Verified 2026-07-31 by signing the same
manifest both ways against the operator-pinned key: 4-field verified locally, 5-field did not,
and the hub only accepts the 5-field form.
"""
...
class Signer:
    ...
    # --- manifest (5-field canonical, matches the live hub) ----------------
    ...
    def manifest_canonical(self, manifest: dict[str, Any]) -> str:
        ...
        # signature or a relaying hub could rewrite trust and prices without breaking it.
        # A manifest with no by_hub (which is platon's normal case — it is a leaf, not a
        # broker) hashes the empty object, exactly as the hub does, so the two agree.
