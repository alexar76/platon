...
/** UI metrics refresh cap — canvas reads telemetryRef every frame without React churn. */
const UI_TELEMETRY_MS = 100;

export function usePlatonSocket() {
  const [telemetry, setTelemetry] = useState<Telemetry | null>(null);
  const telemetryRef = useRef<Telemetry | null>(null);
  const uiTelemetryAt = useRef(0);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.type === "state") {
        telemetryRef.current = msg.data as Telemetry;
        const now = performance.now();
        if (now - uiTelemetryAt.current >= UI_TELEMETRY_MS) {
          uiTelemetryAt.current = now;
