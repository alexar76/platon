...
import { getGraphicsProfile, type GraphicsProfile } from "../lib/graphicsProfile";
...
function useLiveDynamics(telemetryRef: RefObject<Telemetry | null>) {
  const demo = useMemo(() => {
...
function OscillatorField({ telemetryRef }: { telemetryRef: RefObject<Telemetry | null> }) {
  const group = useRef<THREE.Group>(null);
  const ring = useRef<THREE.Mesh>(null);
  const live = useLiveDynamics(telemetryRef);
...
function Effects({ profile }: { profile: GraphicsProfile }) {
  if (!profile.postprocess) return null;
  return (
...
export function ShadowCanvas({
  ...
  const profile = useMemo(() => getGraphicsProfile(), []);
  const [sp0, sp1, sp2] = profile.sparkles;
  ...
  return (
    <Canvas
      ...
      dpr={profile.dpr}
      {...(profile.adaptiveDpr
        ? { performance: { min: 0.85, max: 1, debounce: 250 } }
        : {})}
      resize={{
        scroll: false,
        debounce: { scroll: 50, resize: profile.resizeDebounceMs },
        offsetSize: profile.tier !== "full",
      }}
      frameloop="always"
      gl={{
      ...
      onCreated={({ gl }) => {
        ...
        const cap = profile.tier === "android" ? 1 : profile.tier === "mobile" ? 1.25 : 2;
        gl.setPixelRatio(Math.min(window.devicePixelRatio, cap));
      }}
    >
      <color attach="background" args={["#020208"]} />
      <fog attach="fog" args={["#04030f", 16, 46]} />
      <ambientLight intensity={profile.postprocess ? 0.12 : 0.17} />
      <pointLight position={[8, 10, 6]} intensity={profile.postprocess ? 2.5 : 3} color="#6ee7ff" />
      <pointLight position={[-6, 5, -4]} intensity={profile.postprocess ? 1.8 : 2.2} color="#c084fc" />
      <pointLight position={[0, -2, 8]} intensity={profile.postprocess ? 0.6 : 0.85} color="#f472b6" />
      <Nebula />
      <Stars radius={120} depth={60} count={profile.starsFar} factor={4} fade speed={0.6} />
      {profile.starsNear > 0 && (
        <Stars radius={60} depth={28} count={profile.starsNear} factor={3} fade speed={1.4} />
      )}
      <Sparkles
        count={sp0}
        scale={[16, 9, 16]}
      ...
      <Sparkles
        ...
        color="#f472b6"
      />
      <OscillatorField telemetryRef={telemetryRef} />
      ...
      <OrbitControls
        ...
        minDistance={6}
        autoRotate
        autoRotateSpeed={profile.tier === "android" ? 0.28 : 0.35}
        enableDamping={profile.tier === "android"}
        dampingFactor={0.08}
      />
    </Canvas>
  );
