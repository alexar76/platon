...
**Unpredictability + openable commitment.** As originally shipped the chaos draw was a
*deterministic* function of public inputs (a PRG, not a TRNG). Fresh OS-CSPRNG entropy is
now mixed into every `platon.random`/beacon seed, published as a signed `entropy_commitment`
**and revealed** (the `entropy` preimage) in the proof. `verify_randomness` opens the
commitment (`sha256(entropy) == entropy_commitment`) and re-derives `random_hex` from the
revealed entropy — so a *signed-but-fabricated* output (or a commitment that opens to
nothing) is rejected. The output is unpredictable before issuance and independently
**reproducible from the revealed entropy** after — which is precisely what makes a draw
verifiable rather than "trust the signature." (A bare signature over an unopened commitment
proves only that the operator signed those bytes, not that they came from the entropy.)
