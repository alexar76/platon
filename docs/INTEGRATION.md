...
```bash
export AIMARKET_SEED_LIST="https://platon.example.com/.well-known/ai-market.json"
```
...
result = hub.invoke(
    ...
    channel_id=ch,
    source_hub="https://platon.example.com",
)
...
nodes:
  - id: platon
    label: Platon UMBRAL
    type: shadow-oracle
    url: https://platon.example.com/api/health
    capabilities:
      - platon.state@v1
      - platon.oracle@v1
```
